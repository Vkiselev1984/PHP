...
## ActionLogin of the UserController class

Add invalid data handling and improve its robustness. We will add validation to check for the presence of a login and password, as well as handling possible errors.
...
if (empty($login) || empty($password)) {
error_log("Login or password is empty.");
return $this->render->renderPageWithForm(
'user-auth.twig',
[
'title' => 'Login form',
...
$result = Application::$auth->proceedAuth($login, $password);
} else {
...
- Login and password validation: We added a check for empty values ​​for the login and password. If one of the fields is empty, an error message is returned.
- Using trim: Remove extra spaces from the login and password before checking.
- Error logging: Added log messages to track errors and execution status.
